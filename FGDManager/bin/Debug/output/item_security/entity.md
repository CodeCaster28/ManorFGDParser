...
<div class="column entityentry" markdown="1">This works exactly the same way as [item_generic](../item_generic) does, except that this entity will automatically remove itself from the game when touched, so it allows to create custom collectibles by specifying custom model name.</div>
</div>
</div>
###Keyvalues
<hr>
