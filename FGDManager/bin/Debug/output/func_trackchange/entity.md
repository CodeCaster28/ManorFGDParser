...
Set name of {{ entname }} so other entities can trigger it to move platform up and down. If tracktrain is on platform it will travel with it as well. Every trigger use-type works uniformly. Can be kill-targeted.
</div>
<div class="accordion entityentry">
...
Travel distance, from top to bottom. If "Start at Bottom" flag is selected, the travel distance won't change from bottom to top, it only makes platform start at bottom and awaits for [func_tracktrain](../func_tracktrain) to arrive.
</div>
<div class="entityentry" markdown="1">
...
Name of the [func_tracktrain](../func_tracktrain) this platform will transport to top/bottom track.
</div>
<div class="entityentry" markdown="1">
...
Speed in which {{ entname }} travel the whole way up/down (units per seconds).
</div>
<div class="entityentry" markdown="1">
<span style="color:#9fc5e8;"><b>Blocked Sound</b></span> <kbd  class="tooltip" data-tooltip="sound">noise2</kbd> :
Sound file to use when the {{ entname }} is blocked. This overrides default "Stop Sound" sound. Note that other sounds format (AIFF, ASF, DLS, FLAC, IT, M3U, MID, MOD, MP2, MP3, OGG, S3M, VAG, WMA, XM) can be used, even if editor sound browser allows to pick only WAV files.
</div>
<div class="accordion entityentry">
