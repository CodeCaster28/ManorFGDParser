...
Set name of {{ entname }} so other entities can trigger it to turn wall on or off. Depending on trigger use-type: 'On'- turn on (hide), 'Toggle'- toggle, 'Off'- turn off (show). Can be kill-targeted.
</div>
<div class="entityentry" markdown="1">
...
<ul>
<li class="imagepadding" markdown="1"><b>1 </b> : <span style="color:#9fc5e8;">Starts Invisible</span> : Func_wall is turned off (invisible) at map start. By default it's visible at start.</li>
